    ...
    async (username: string, password: string): Promise<boolean> => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          try {
            ...
            if (sResp.ok) {
              ...
              if (servers.length > 0 && servers[0].id) {
                ...
                Cookies.set("nx_server_id", servers[0].id, {
                  expires: 365,
                  path: "/",
                });
              }
          ...
          setSession(user);
          Cookies.set("local_nx_user", JSON.stringify(user), {
            expires: 28,
            path: "/",
          });
          return true;
    ...
    try {
      ...
      if (tokens.access_token) {
        ...
        if (systemsResp.ok) {
          ...
          const errBody = await systemsResp.json().catch(() => ({}));
          console.error(
            "[NxCloud] Failed to fetch systems:",
            systemsResp.status,
            errBody,
          );
        }

        const ownerSystem = systemsList.find(
          (s: any) =>
        ...
        const ownerSystemId =
          ownerSystem?.id ||
          (systemsList.length > 0 ? systemsList[0].id : undefined);

        if (ownerSystemId) {
          Cookies.set("nx_system_id", ownerSystemId, {
            expires: 365,
            path: "/",
          });
        }
