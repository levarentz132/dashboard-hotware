...
import { useAlarms } from "@/hooks/useNxAPI";
...
export default function RecentAlarmsWidget() {
  ...
  const recentAlarms = alarms.slice(0, 5).map((alarm) => {
    ...
    return {
      ...
      severity: alarm.type.toLowerCase().includes("offline")
        ? "critical"
        : alarm.type.toLowerCase().includes("motion")
        ? "high"
        : alarm.type.toLowerCase().includes("tamper")
        ? "medium"
        : "low",
    ...
    <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          {activeAlarmsCount === 0 && !loading && <span className="text-sm text-gray-500">No active alarms</span>}
        </div>
      </div>

      <div className="space-y-4">
        {loading && <div className="text-center py-4 text-gray-500">Loading alarms...</div>}

        {error && !loading && <div className="text-center py-4 text-red-600">Unable to load alarms</div>}

        {!loading && !error && recentAlarms.length === 0 && (
          <div className="text-center py-4 text-gray-500">No recent alarms</div>
        )}
      ...
      <div className="mt-4 pt-4 border-t border-gray-100">
        <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">View alarm console →</button>
      </div>
    </div>
  );
}
