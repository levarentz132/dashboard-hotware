      ...
      <div className="space-y-4">
        <div className="text-sm text-gray-600">
          ...
          <div className="mt-1 bg-gray-50 p-2 rounded text-xs font-mono">
            Base URL: {API_CONFIG.baseURL}<br/>
            Username: {API_CONFIG.username}<br/>
          ...
          <div>
            ...
            <div className="border rounded p-3 bg-gray-50">
              <div className={`text-sm font-medium ${
                systemInfoResult.status.includes('200') 
                  ? 'text-green-600' 
                  : 'text-red-600'
              }`}>
              ...
              {systemInfoResult.error && (
                <div className="text-xs text-red-600 mt-1">{systemInfoResult.error}</div>
              )}
              {systemInfoResult.data && (
                <div className="mt-3">
                  ...
                  <div className="bg-white p-3 rounded border text-xs space-y-1">
                    <div><strong>Name:</strong> {systemInfoResult.data.name || 'N/A'}</div>
                    <div><strong>Version:</strong> {systemInfoResult.data.version || 'N/A'}</div>
                    <div><strong>Customization:</strong> {systemInfoResult.data.customization || 'N/A'}</div>
                    <div><strong>Local ID:</strong> {systemInfoResult.data.localId || 'N/A'}</div>
                    <div><strong>Servers:</strong> {systemInfoResult.data.servers?.length || 0}</div>
                    <div><strong>Devices:</strong> {systemInfoResult.data.devices?.length || 0}</div>
                    <details className="mt-2">
                      <summary className="cursor-pointer text-blue-600 hover:text-blue-800 text-xs">View Raw JSON</summary>
                      <pre className="mt-2 text-xs bg-gray-100 p-2 rounded overflow-x-auto">{JSON.stringify(systemInfoResult.data, null, 2)}</pre>
                    </details>
                  </div>
                </div>
              )}
        ...
        {serversResult && (
          <div>
            ...
            <div className="border rounded p-3 bg-gray-50">
              <div className={`text-sm font-medium ${
                serversResult.status.includes('200') 
                  ? 'text-green-600' 
                  : 'text-red-600'
              }`}>
              ...
              {serversResult.error && (
                <div className="text-xs text-red-600 mt-1">{serversResult.error}</div>
              )}
              {serversResult.data && (
                <div className="mt-3">
                  ...
                  <div className="bg-white p-3 rounded border text-xs">
                    <details>
                      <summary className="cursor-pointer text-blue-600 hover:text-blue-800 text-xs">View Server Data JSON</summary>
                      <pre className="mt-2 text-xs bg-gray-100 p-2 rounded overflow-x-auto">{JSON.stringify(serversResult.data, null, 2)}</pre>
                    </details>
                  </div>
                </div>
              )}
